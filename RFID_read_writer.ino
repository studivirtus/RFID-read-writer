...
uint8_t irqq = 2;
uint8_t resett = 3;
Adafruit_NFCShield_I2C nfc(irqq, resett);

class RFIDDataReader
{
	public:
		RFIDDataReader();
		void NFCShieldSetup();
...
void RFIDDataReader::NFCShieldSetup()
{
...
void RFIDDataReader::cardReader()
{
        ...
        for (currentblock = 0; currentblock < 64; currentblock++)
        {
            ...
            if (success)
            {
                /*	Serial.print("dataIndex : ");
                    ...
                    Serial.print("\n");*/
                for ( int i = 0 ; i < 16 && dataIndex < this.len_data_2; i++, dataIndex++ )
                {
                    data2[dataIndex] = data[i];
                /*	Serial.print(data2[dataIndex],HEX);
                    Serial.print(" ");
                    Serial.print(data[i],HEX);
                    Serial.print("\n");*/
                }
            /*	Serial.print("Test of Data_2\n");
    ...
    for(int i = 0; i < this->len_data_2; i++)
    {
        if ((i % 16) > 0)
        {
            Serial.print(data2[i],HEX);
    ...
    RFIDDataReader idReader;

    void setup(void) {

    idReader.NFCShieldSetup();
    /* // has to be fast to dump the entire memory contents!
    Serial.begin(115200);
    Serial.println("Looking for PN532...");
    nfc.begin();
	...
	for (currentblock = 0; currentblock < 64; currentblock++)
	{
		...
		if (success)
		{
			/*	Serial.print("dataIndex : ");
				Serial.print(dataIndex,DEC);
				Serial.print("\n");*
			for ( int i = 0 ; i < 16 && dataIndex < len_data_2; i++, dataIndex++ )
			{
				data2[dataIndex] = data[i];
			/*	Serial.print(data2[dataIndex],HEX);
				Serial.print(" ");
